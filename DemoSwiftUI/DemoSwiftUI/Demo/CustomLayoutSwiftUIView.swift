...
struct CollectSize: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content.background(GeometryReader { proxy in
            Color.clear
                .preference(key: CollectSizePreference.self, value: [self.index: proxy.size]) })
            Text("(\(index))")
    }
}
...
struct Stack<Element, Content: View>: View {
    ...
    var body: some View { ZStack(alignment: alignment) {
...
struct CustomLayoutSwiftUIView: View {
    ...
    var body: some View {
        VStack {
            ...
            }) { Text("Toggle axis\(horizontal ? "[O]" : "[X]")") }
...
struct CustomLayoutSwiftUIView_Previews: PreviewProvider {
    static var previews: some View {
        CustomLayoutSwiftUIView()
    }
}
